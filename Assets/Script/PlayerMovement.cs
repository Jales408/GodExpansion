...
public class PlayerMovement : MonoBehaviour {
    ...
    public void Move(float _move, bool _jump,bool _defend)
    {
        ...
        if ((m_Grounded || m_AirControl))
        {
            if (!(m_Grounded && _defend)) {
                ...
                // Move the character
                m_Rigidbody2D.velocity = new Vector2(_move * m_MaxSpeed * 2, m_Rigidbody2D.velocity.y);
            }
    ...
    {
        Instantiate(bloodEffect, swordPos.position, Quaternion.identity);
        return damage;
    }
}
